...
import { CheckCircle, Copy, Check, RefreshCw, ArrowRight, Image as ImageIcon, Type, Sparkles, Smartphone, FileEdit, Download } from "lucide-react";
import { cleanStep4ThumbnailJson } from "../utils/jsonCleaner";

interface Step4ThumbnailProps {
  content: string;
...
export const Step4Thumbnail: React.FC<Step4ThumbnailProps> = ({
  content,
  isGenerating,
  onApproveAndNext,
  onRegenerate,
  ...
  const handleCopy = () => {
    const jsonToCopy = cleanStep4ThumbnailJson(content) || content;
  ...
  const handleDownloadJson = () => {
    const jsonStr = cleanStep4ThumbnailJson(content) || content;
    const blob = new Blob([jsonStr], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `thumbnail_prompt_${Date.now()}.json`;
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base font-bold text-slate-100">
              BƯỚC 4: PROMPT TẠO THUMBNAIL TỶ LỆ 9:16 "STOP-THE-SCROLL"
            </h3>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Prompt tạo ảnh bìa bắt mắt • 3 Tiêu đề Text Overlay ngắn đắt giá (&lt; 6 chữ) • Phối màu tương phản cao
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            title="Tải về file Thumbnail Prompt JSON"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-start space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-200">Tỷ lệ 9:16 & Quy tắc an toàn (Safe Zone)</h4>
            <p className="text-slate-400 mt-0.5">
              Chừa trống 20% trên và dưới để tránh bị che bởi UI TikTok/Reels/Shorts.
            </p>
          </div>
        </div>

        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-start space-x-3">
          <Type className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
          <div>
            <h4 className="font-semibold text-slate-200">Text Overlay gây tò mò &lt; 6 từ</h4>
            <p className="text-slate-400 mt-0.5">
              Font chữ đậm, màu vàng/trắng viền đen, tạo cảm giác cấp bách hoặc giải pháp bất ngờ.
            </p>
          </div>
        </div>
      </div>
        ...
        {isEditing ? (
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">
              Chỉnh sửa Prompt Thumbnail & Text Overlays:
            </label>
            <textarea
              id="textarea-step4-edit"
              rows={14}
              value={content}
          ...
          >
            {content || (
              <div className="text-slate-500 italic py-8 text-center">
                Đang chờ tạo dữ liệu Thumbnail 9:16 từ hệ thống AI...
              </div>
            )}
          </div>
        )}
      ...
      <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold text-amber-300">
            ❓ "Bạn có duyệt Thumbnail & Tiêu đề này không để tôi qua Bước 5 xuất JSON kịch bản tổng thể?"
          </p>
          <p className="text-[11px] text-slate-400 mt-0.5">
            Nhấn Duyệt bên dưới để chuyển tiếp sang Bước 5: Đóng gói JSON kịch bản hoàn chỉnh cho N8N / Make / CapCut
          </p>
        </div>

        <button
          id="btn-approve-step4"
        ...
        >
          <span>Đồng ý / Duyệt & Đi tiếp Bước 5</span>
